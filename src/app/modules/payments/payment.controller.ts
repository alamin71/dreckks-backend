...
// ----------------------
 const saveCard = async (req: Request, res: Response) => {
  try {
    ...
    res.status(500).json({ success: false, message: 'Error saving card', error });
  }
};
...
// ----------------------
 const getSavedCards = async (req: Request, res: Response) => {
  try {
    ...
    res.status(500).json({ success: false, message: 'Failed to fetch cards', error });
  }
};
...
// ----------------------
 const makePayment = async (req: Request, res: Response) => {
  try {
    const { userId, subscriptionId, amount, savedCardId } = req.body;
